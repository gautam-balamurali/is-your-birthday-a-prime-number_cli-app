...
function welcome() {
  let inputName = capitalizeFirstLetter(getAnswer('\nEnter your name: ', { defaultInput: 'nobody' }));
  let dob = getAnswer(`\n Hello ${nameBg(inputName)}, enter your year of birth in DD/MM format: `);
...
function isDOBValid(dob, name) {
  if (!dob.includes('/')) {
    log(`\n${no('Please enter your year of birth in DD/MM format.')}`);
    playAgain();
    return;
  }
  dob = dob.split('/');
...
function displayResult(name, isPrimeNumber) {
  if (isPrimeNumber) {
    log(`\n${yes(`Yes ${name}, your birth day is a prime number. You should share it on your social handles.`)}`);
  } else {
